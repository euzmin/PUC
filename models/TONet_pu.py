...
class TONet_pu(nn.Module):
    ...
    def __init__(self, input_dim, h_dim, num_layers) -> None:
        ...
        for i in range(1, len(self.layer_cfg)):
            self.x_net.append(nn.ELU())
            self.x_net.append(nn.Linear(self.layer_cfg[i-1], self.layer_cfg[i]))
        self.x_net = nn.Sequential(*self.x_net)
        self.t_net = nn.Linear(self.layer_cfg[-1], 1)
        self.y_net = nn.Linear(self.layer_cfg[-1], 1)
        self.y_bce = nn.BCELoss()
        self.t_bce = nn.BCELoss()
    ...
    def forward(self, x):
        x_rep = self.x_net(x)
        t_out = self.t_net(x_rep)
        y_out = self.y_net(x_rep)

        t_hat = torch.sigmoid(t_out)
        y_hat = torch.sigmoid(y_out)
        eps = self.epsilon(torch.ones_like(t_hat)[:, :1])

        return y_hat, t_hat, eps, y_out

    def criterion(self, y_hat, t_hat, eps, y_out, tau_hat, y, t, alpha, beta, gamma, w=1.0):
        # loss = (w*(out.squeeze()-y.squeeze())**2).mean()
        y_loss = self.y_bce(y_hat.squeeze(), y.squeeze()).mean()
        t_loss = self.t_bce(t_hat.squeeze(), t.squeeze()).mean()
        to_loss = y_loss + alpha * t_loss
        
        t_hat = ((t_hat+ 0.01)/1.02).squeeze()

        h = (t.squeeze() / t_hat) - ((1 - t.squeeze()) / (1 - t_hat))
        ...
        return to_loss + beta * targeted_regularization + gamma * pu_pairwise_loss
    def predict(self, x):
        ...
        with torch.no_grad():
            y_0, t_0, eps, y_out = self.forward(x_t0)
            y_1, t_1, eps, y_out = self.forward(x_t1)
    ...
    def train_model(self, opt, train_dataloader, valid_data, test_data, args, exp=None, best_model_path=None):
        ...
        for epoch in range(args.epochs):
            for i, sample in enumerate(train_dataloader):
                ...
                t1 = torch.ones(t.shape[0], 1)
                t0 = torch.zeros(t.shape[0], 1)
                xt1 = torch.cat([x, t1], dim=1)
                xt0 = torch.cat([x, t0], dim=1)
                inputs = torch.cat([xt, xt0, xt1], dim=0)
                y_hat, t_hat, eps, y_out = self(inputs)

                tau_hat = y_hat[2*y.shape[0]:] - y_hat[y.shape[0]:2*y.shape[0]]


                y_hat = y_hat[:y.shape[0]]
                t_hat = t_hat[:t.shape[0]]
                eps = eps[:t.shape[0]]
                y_out = y_out[:t.shape[0]]

                loss = self.criterion(y_hat, t_hat, eps, y_out, tau_hat, y, t, args.alpha, args.beta, args.gamma, w)
                loss.backward()
                opt.step()
