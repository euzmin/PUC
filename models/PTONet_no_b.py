...
class PTONet_no_b(nn.Module):
    ...
    def criterion(self, y_hat, t_recon, e_hat, eps, y_out, tau_hat, y, t, alpha, beta, w=1.0):
        ...
        return to_loss + alpha * targeted_regularization + beta * pu_pairwise_loss
    def predict(self, x):
    ...
    def train_model(self, opt, train_dataloader, valid_data, test_data, args, exp=None, best_model_path=None):
        ...
        for epoch in range(args.epochs):
            for i, sample in enumerate(train_dataloader):
                ...
                tri_x = torch.cat([x,x,x], dim=0)
                tri_t = torch.cat([t.unsqueeze(-1),t0,t1], dim=0)
                y_hat, t_hat, e_hat, eps, y_out = self(tri_x,tri_t)
                tau_hat = y_hat[2*y.shape[0]:] - y_hat[y.shape[0]:2*y.shape[0]]
                y_hat = y_hat[:y.shape[0]]
                t_hat = t_hat[:t.shape[0]]
                e_hat = e_hat[:t.shape[0]]
                eps = eps[:t.shape[0]]
                y_out = y_out[:y.shape[0]]
                loss = self.criterion(y_hat, t_hat, e_hat, eps, y_out, tau_hat, y, t, args.alpha, args.beta, w)
                loss.backward()
                opt.step()
